...
% outout the 1st stage?
clear all; close all;

addpath('../../matlab/');
addpath('../../utils/');
caffe.reset_all();
root_dir = 'mobile1_atten/';
...
definition_file = [root_dir 'mscnn_deploy_1st.prototxt'];
...
%    fclose(ffid);  
  ...
  tmp=squeeze(outputs{1}); cls_pred = tmp'; 
  tmp=squeeze(outputs{1}); tmp = tmp'; tmp = tmp(:,2:end); 
  ...
  for i = 1:num_cls
    ...
    bbset = double([tx ty tw th ones(length(tx),1)]);
    bbset_my=bbset;
%     bbset_my2=zeros(0,5);
%     [m1 n1]=size(bbset_my);
%     for i1=1:m1
    ...
    if (show) 
      ...
      if (size(bbset,1)>0) 
        ...
        bbs_show = bbset(show_id,:);
         proposals_show = proposals(:,:); 
      end
      % proposal
      for j = 1:size(proposals_show,1)
        rectangle('Position',proposals_show(j,1:4),'EdgeColor','g','LineWidth',0.001);
