...
root_dir = 'mscnn_ori/';
binary_file = [root_dir 'mscnn_ori_LightOrNot.caffemodel'];
assert(exist(binary_file, 'file') ~= 0);
definition_file = [root_dir 'mscnn_deploy_sub.prototxt'];
...
root_dir = '/media/b3-542/LIBRARY/moz/dataset/images/';
%root_dir = '/media/b3-542/LIBRARY/moz/dataset/HK_dataset/Hong-Kong-nighttime-vehicle-dataset0/';
...
for k = 1:nImg
  ...
  for i = 1:num_cls
    ...
    bbset = [bbset_temp ;double([tx ty tw th prob])];
    bbset_temp=bbset;
...
for j=1:num_cls
  ...
  dlmwrite(['detections/' comp_id '_' 'car' '.txt'],save_detect_boxes);
end
